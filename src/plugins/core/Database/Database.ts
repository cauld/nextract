...
import { has, isArray, isUndefined, isEmpty, merge } from 'lodash/fp';
...
function buildNewConnection(dbName) {
  ...
  let dbpluginConfig: any = databasePlugin.ETL.config.databases[dbName];

  let dbConfigObject: IDbConfigObject = {
    debug: has(dbpluginConfig, 'debug') ? dbpluginConfig.debug : false,
  ...
  if (dbpluginConfig.client === 'pg' && has(dbpluginConfig, 'searchPath')) {
...
function getInstance(dbName) {
  if (!has(connectionInstances, dbName)) {
...
module.exports = {
  ...
  joinQuery: function(dbName, sqlStatement, joinFilterColumns, joinColumnsToReturn = [], returnedUnmatched = true) {
    ...
    let streamFunction = (element, encoding, callback) => {
      if (isEmpty(element)) {
        this.push(element);
        return callback();
      } else {
        let joinValues: string[] = [];
        joinFilterColumns.forEach((prop) => {
        ...
        dbInstance.raw(sqlStatement, joinValues)
          .then((joinData) => {
            ...
            if (joinData[0].length === 1) {
              ...
              element = merge(element, joinData[0][0]);
              ...
              if (isArray(joinColumnsToReturn) && joinColumnsToReturn.length > 0) {
  ...
  insertQuery: function(dbName, tableName, batchSize = 1000, queueSize = 10) {
    let dbInstance = getInstance(dbName);
    let elementsToInsert: string[] = [];
    let stream: any = null;

    let q = async.queue((element, callback) => {
      //Add to the batch
    ...
    function processStreamInput(this: any, element, encoding, callback) {
      stream = this;
  ...
  updateQuery: function(dbName, tableName, columnsToUpdate, matchCriteria = []) {
    ...
    function processStreamUpdate(element, encoding, callback) {
      if (!isUndefined(element)) {
        ...
        let updateObj = {};
        if (isArray(columnsToUpdate) && !isEmpty(columnsToUpdate)) {
