...
function checkInput(element, propertyToProcess) {
  if (isUndefined(element[propertyToProcess]) || !isNumber(element[propertyToProcess])) {
...
module.exports = {
  ...
  */
  maxBy: function(propertyToProcess) {
    function checkAgainstMax(element, encoding, callback) {
      ...
      if (isUndefined(this.currentMax) || (element[propertyToProcess] > this.currentMax)) {
  ...
  */
  minBy: function(propertyToProcess) {
    function checkAgainstMin(element, encoding, callback) {
      ...
      if (isUndefined(this.currentMin) || (element[propertyToProcess] < this.currentMin)) {
  ...
  */
  meanBy: function(propertyToProcess) {
    ...
    function flushGroupBy(callback) {
      let calculatedMean = mean(valuesToMean);
  ...
  */
  uniqBy: function(propertyToProcess, returnElement = false) {
    ...
    function flushGroupBy(callback) {
      ...
      if (returnElement === true) {
        uniques = uniqBy(valuesToUnique, propertyToProcess);
      } else {
        uniques = uniq(valuesToUnique);
      }
