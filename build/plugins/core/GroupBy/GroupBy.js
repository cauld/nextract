...
let groupByPlugin = new pluginBase_1.default('GroupBy', 'Core');
function checkInput(element, propertyToProcess) {
    if (fp_1.isUndefined(element[propertyToProcess]) || !fp_1.isNumber(element[propertyToProcess])) {
...
module.exports = {
    ...
    maxBy: function (propertyToProcess) {
        function checkAgainstMax(element, encoding, callback) {
            ...
            if (fp_1.isUndefined(this.currentMax) || (element[propertyToProcess] > this.currentMax)) {
    ...
    minBy: function (propertyToProcess) {
        function checkAgainstMin(element, encoding, callback) {
            checkInput(element, propertyToProcess);
            if (fp_1.isUndefined(this.currentMin) || (element[propertyToProcess] < this.currentMin)) {
    ...
    sumBy: function (propertyToProcess) {
        function combine(element, encoding, callback) {
            checkInput(element, propertyToProcess);
            if (fp_1.isUndefined(this.total)) {
    ...
    meanBy: function (propertyToProcess) {
        ...
        function flushGroupBy(callback) {
            let calculatedMean = fp_1.mean(valuesToMean);
    ...
    uniqBy: function (propertyToProcess, returnElement = false) {
        ...
        function flushGroupBy(callback) {
            let uniques;
            if (returnElement === true) {
                uniques = fp_1.uniqBy(valuesToUnique, propertyToProcess);
            }
            else {
                uniques = fp_1.uniq(valuesToUnique);
            }
