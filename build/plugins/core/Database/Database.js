...
databasePlugin = new pluginBase_1.default('Database', 'Core');
...
function buildNewConnection(dbName) {
    ...
    let dbConfigObject = {
        debug: fp_1.has(dbpluginConfig, 'debug') ? dbpluginConfig.debug : false,
    ...
    if (dbpluginConfig.client === 'pg' && fp_1.has(dbpluginConfig, 'searchPath')) {
    ...
    if (fp_1.has(dbpluginConfig, 'pool')) {
...
function getInstance(dbName) {
    if (!fp_1.has(connectionInstances, dbName)) {
...
module.exports = {
            ...
            if (fp_1.isEmpty(element)) {
                ...
                dbInstance.raw(sqlStatement, joinValues)
                    ...
                    if (joinData[0].length === 1) {
                        ...
                        element = fp_1.merge(element, joinData[0][0]);
                    }
                        ...
                        if (fp_1.isArray(joinColumnsToReturn) && joinColumnsToReturn.length > 0) {
    ...
    insertQuery: function (dbName, tableName, batchSize = 1000, queueSize = 10) {
        ...
        let stream = null;
        let q = async_1.default.queue((element, callback) => {
            //Add to the batch
    ...
    updateQuery: function (dbName, tableName, columnsToUpdate, matchCriteria = []) {
        ...
        function processStreamUpdate(element, encoding, callback) {
            if (!fp_1.isUndefined(element)) {
                ...
                let updateObj = {};
                if (fp_1.isArray(columnsToUpdate) && !fp_1.isEmpty(columnsToUpdate)) {
