...
let sortPlugin = new pluginBase_1.default('Sort', 'Core');
...
module.exports = {
    ...
    sortIn: function (propertiesToSortBy, ordersToSortBy) {
        if (!fp_1.isArray(propertiesToSortBy) || !fp_1.isArray(ordersToSortBy) || propertiesToSortBy.length !== ordersToSortBy.length) {
        ...
        */
        ...
        let batchSize = 100;
        let q = async_1.default.queue(function (element, callback) {
            if (fp_1.isNull(tableName)) {
                //Add to the batch
    ...
    sortOut: function (sortInDbInfo) {
        ...
        */
        ...
        let toObjectStream = (chunk, encoding, callback) => {
            if (!fp_1.isUndefined(chunk) && !fp_1.isNull(chunk)) {
                let oStream = this;
                ...
                splitChunkStrings.map(function (cStr) {
                    if (!fp_1.isEmpty(cStr) && cStr.charAt(cStr.length - 1) === '}') {
