...
transform.loadPlugins('Core', ['Database', 'Logger']).then(function (dbDataStream) {
  var sql = 'select first_name, last_name from employees where age >= ?';
  var sqlReplacements = [30];

  transform.Plugins.Core.Database.selectQuery('nextract_sample', sql, sqlReplacements).on('data', function (resultingData) {
